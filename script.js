(() => {
  ...
  if (homeCards && window.renderProjectCard) {
    const featured = projects.filter(({ id }) => ["spatial-experiment", "editorial-system", "digital-product", "visual-identity"].includes(id));
    homeCards.innerHTML = featured.map((project, index) => window.renderProjectCard(project, index)).join('');
  }
  ...
  if (workCards && window.renderProjectCard) {
    ...
    document.querySelectorAll('[data-filter]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        document.querySelectorAll('.project-tile').forEach((card) => card.classList.add('is-visible'));
      });
    });
  }
  ...
  if (projectPage) {
    ...
    if (!project) {
      ...
      const setImage = (nextIndex) => {
        imageIndex = (nextIndex + project.images.length) % project.images.length;
      ...
      galleryFrame.addEventListener('click', (event) => {
        if (event.target === galleryCursor || window.matchMedia('(pointer: fine)').matches) return;
        const rectangle = galleryFrame.getBoundingClientRect();
        setImage(imageIndex + (event.clientX - rectangle.left < rectangle.width / 2 ? -1 : 1));
      });
    }
  }
  ...
  document.querySelectorAll('a[href]').forEach((link) => {
    ...
    const isLocalPage = url.origin === window.location.origin && /(?:index|work|about|project)\.html$/.test(url.pathname);
